// // SPDX-License-Identifier: MIT

pragma solidity 0.8.13;

import {Test, console} from "forge-std/Test.sol";
import {StdCheats} from "forge-std/StdCheats.sol";
import {MoodDynamicNFT} from "../src/MoodDynamicNFT.sol";
import {DeployMoodDynamicNFT} from "../script/DeployMoodDynamicNFT.s.sol";

contract MoodDynamicNFTIntegrationTest is StdCheats, Test {
    MoodDynamicNFT moodNFT;
    DeployMoodDynamicNFT public deployer;

    string public constant HAPPY_SVG_IMAGE_URI =
        "data:image/svg+xml;base64,";

    string public constant SAD_SVG_IMAGE_URI =
        "data:image/svg+xml;base64,";

    //     string public constant EXCITED_SVG_URI =
    //         "data:image/svg+xml;base64,";

    //     string public constant NEUTRAL_SVG_URI =
    // "data:image/svg+xml;base64,";

    address public constant MINTER = address(1);

    address USER = makeAddr("user");

    function setUp() public {
        deployer = new DeployMoodDynamicNFT();
        moodNFT = deployer.run();
    }

    function testForMoodStatusIsHappy() public {
        vm.prank(MINTER);

        moodNFT.mintNft();

        vm.prank(MINTER);

        assertEq(
            keccak256(abi.encodePacked(moodNFT.getCurrentMood(0))),
            keccak256(abi.encodePacked(MoodDynamicNFT.MOOD.HAPPY))
        );
    }

    function testShouldRevertIfChangeIsToSameMoodAlreadyIn() public {
        vm.prank(MINTER);

        moodNFT.mintNft();

        vm.prank(MINTER);
        vm.expectRevert();

        moodNFT.changeMood(0, "HAPPY"); // should revert
        assertEq(
            keccak256(abi.encodePacked(moodNFT.getCurrentMood(0))),
            keccak256(abi.encodePacked(MoodDynamicNFT.MOOD.HAPPY))
        );

        console.log(moodNFT.tokenURI(0));
    }

    function testChangeMoodToDifferentMood() public {
        vm.prank(USER);

        moodNFT.mintNft();

        vm.prank(USER);

        moodNFT.changeMood(0, "EXCITED");
        assertEq(
            keccak256(abi.encodePacked(moodNFT.getCurrentMood(0))),
            keccak256(abi.encodePacked(MoodDynamicNFT.MOOD.EXCITED))
        );

        console.log(moodNFT.tokenURI(0));
    }

    function testShouldRevertForAnInvalidMood() public {
        vm.prank(USER);

        moodNFT.mintNft();

        vm.prank(USER);
        vm.expectRevert(); // should revert

        moodNFT.changeMood(0, "EXTATIC");
        assertEq(
            keccak256(abi.encodePacked(moodNFT.getCurrentMood(0))),
            keccak256(abi.encodePacked(MoodDynamicNFT.MOOD.HAPPY))
        );

        console.log(moodNFT.tokenURI(0));
    }
}
